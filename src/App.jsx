...
function App() {
    ...
    const [currentView, setCurrentView] = useState(() => {
        ...
        if (path === '/demo/admin' || path === '/admin' || path === '/demo') return 'dashboard'
        if (path === '/demo/store' || path === '/store' || path === '/self') return 'menu'
        if (path === '/settings') return 'settings'
        return 'landing'
    })
    const [isSelfOrder, setIsSelfOrder] = useState(() => window.location.pathname === '/demo/store' || window.location.pathname === '/store' || window.location.pathname === '/self')
    ...
    useEffect(() => {
        const handlePopState = () => {
            ...
            } else if (path === '/demo/store' || path === '/store' || path === '/self') {
                setCurrentView('menu')
                setIsSelfOrder(true)
            } else if (path === '/demo/admin' || path === '/demo' || path === '/admin') {
    ...
    const handleViewChange = (view) => {
        ...
        if (view === 'dashboard') {
            ...
            window.history.pushState({}, '', '/demo/store')
            setIsSelfOrder(true)
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, async (snapshot) => {
            if (snapshot.empty && config.menu && config.menu.length > 0) {
                // One-time migration: If Firestore is empty, upload from config.json
                console.log("Migrating menu from config.json to Firestore...")
    ...
    if (!isSelfOrder && !user && (currentView === 'dashboard' || currentView === 'settings')) {
        return (
    ...
    return (
        <div className="min-h-screen bg-app-bg text-app-text selection:bg-indigo-500/30 font-sans pb-24 transition-colors duration-300">
            ...
            <main className={currentView === 'landing' ? '' : 'max-w-7xl mx-auto py-8'}>
                {currentView === 'landing' && (
                    <LandingView
                        onStart={(target) => {
                            if (target === 'menu') {
                ...
                {currentView === 'settings' && (
                    <SettingsView onBack={() => handleViewChange('dashboard')} />
                )}
            </main>
